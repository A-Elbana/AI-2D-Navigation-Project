...
class Agent:
    def __init__(self) -> None:
        ...
        self.gamma = 0.7  # discount_rate < 1
        self.memory = deque(maxlen=MAX_MEMORY)
        self.model = Linear_QNet(11, 32,4).to('cuda:0')
    ...
    def get_state(self, game: AgarGame):
        ...
        if position[1] == 30:
            wall_up = 1
        intersection = do_lines_intersect((game.pos[0], game.pos[1]), (game.food[0], game.food[1]), (obs.x1,obs.y1),(obs.x2,obs.y2))
        d_to_food = ((game.pos[0] - game.food[0])**2 + (game.pos[1] - game.food[1])**2)**0.5
        state = [round(game.pos[0]/game.w,2), round(game.pos[1]/game.h, 2), round(game.food[1]/game.h, 2), round(game.food[0]/game.w, 2), wall_right, wall_left, wall_up, wall_down, intersection, distance(position, obs)/800, d_to_food/800]  
        game.state = state
